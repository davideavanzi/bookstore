...
module.exports = function (app) {
		...
		try{
			...
			await JSON.stringify(client.query('SELECT id FROM users WHERE "email"=$1', [req.body.username], function(err, result) {
				if(result.rows[0]){
					...
					console.log("User already registered! Username: "+req.body.firstName)
					res.redirect('/join');
				}
				else{
					client.query('INSERT INTO users ("firstName", "lastName", "email", "password") VALUES ($1, $2, $3, $4)', [req.body.firstName, req.body.lastName, req.body.username, pwd], function(err, result) {
						if(err){console.log(err);}
						else {
						
						client.query('COMMIT')
							console.log("User registered! Username: "+req.body.firstName)
	...
	app.get('/account', function (req, res, next) {
		if(req.isAuthenticated()){
			console.log("Already authenticated user id:" + req.user.id + " when perform account");
			...
			console.log("Not already authenticated user, showing login form");
			res.redirect('/login');
		}
	});
	...
	app.get('/login', function (req, res, next) {
		if (req.isAuthenticated()) {
			console.log("Already authenticated user id: " + req.user.id + " when perform login");
	...
	app.get('/logout', function(req, res){
		console.log("user id: "+req.user.id+" was logged in: "+req.isAuthenticated()+" before asking for logout");
		req.logout();
		console.log("user is logged out: "+(!req.isAuthenticated())+" after asking for logoutg");
		...
		passport.authenticate('local', {
	  	...
	  	function(req, res, user) {
	    	if (req.body.remember) {
				console.log("Expiration cookie set 30 days");
				req.session.cookie.maxAge = 30 * 24 * 60 * 60 * 1000; // Cookie expires after 30 days
			} else {
				console.log("Expiration cookie set for this session");
		...
		try{
			...
			var currentAccountsData = await JSON.stringify(client.query('SELECT id, "firstName", "email", "password" FROM users WHERE "email"=$1', [username], function(err, result) {
				...
				if(result.rows[0] == null){
					console.log("Oops. Incorrect login details, wrong username");
					...
					bcrypt.compare(password, result.rows[0].password, function(err, check) {
						if (err){
							...
							console.log("login successful for user id: " + result.rows[0].id);
							return done(null, result.rows[0]);
						}
						else{
							req.flash('danger', "Oops. Incorrect login details.");
							console.log("Oops. Incorrect login details, wrong password");
							return done(null, false);
						}
					});
				}
