...
- **Reservation**: This model represents the willing of a customer to reserve a book which is currently out of stock in the store.
...
![ER diagram](https://hyp.avanzi.dev/backend/er.svg)
...
- *Why do you think your web application adheres to common practices to partition
  ...
  All static assets are served to clients by the node serve-static module. Application data are then exchanged by AJAX requests made by the client.
...
- Davide learnt the JavaScript language and how to run a proper NodeJS server and the principles of a RESTful API. He will further explore this topic in order to use it on future real world projects and works.
